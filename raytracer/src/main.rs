...
fn main() -> IoResult<()> {
    ...
    let material_left:   MaterialPtr = Rc::new(Metal::new(Color::new(0.8, 0.8, 0.8), 0.3)); // + fuzz
    let material_right:  MaterialPtr = Rc::new(Metal::new(Color::new(0.8, 0.6, 0.2), 1.0)); // + fuzz
    ...
    // ---------- Output ----------
    let file = File::create("image14.ppm")?;
